...
if (strcmp(Word,"init")==0)     //~0 init:410; - включение гетеродина 410 МГц                
   {
	 crc_comp =atoi(DATA_Word);
...
//-----------------------
	...
	R8B.TYPE0 = 0;//выход 8, ИНОГДА ВЫКЛЮЧАЕМ -0 ТАК как на оснастке есть 125 генератор //6 - LVCMOS (Norm/Inv) ,1 - lvds
	R8B.TYPE1 = 0;//выход 9,
...
//----------------------
	...
	R28B.ADR      = 28;
    R28B.PLL_R    = 1;  //частота сравнения 10 МГц
...
//-----------------------
	...
	R7B.TYPE0 = 0;//выход 4,
	R7B.TYPE1 = 0;//выход 5,
	R7B.TYPE2 = 5;//выход 6,  //6 - LVCMOS (Norm/Inv) 2 - lvpecl (700 мВ)
	R7B.TYPE3 = 0;//выход 7,
	R7B.ADR   = 7;
...
//-----------------------------------------------------------
// это тест для проверки фильтра sclf-44 , делаем гетеродин 410 МГц из 100 Мгц ref
/*
...
//----------------------
    ...
    R29B.PLL_N_CAL     =41;  // 63 - 20MHz
  
    R30B.ADR           =30;
    R30B.PLL_P         =6;  //  2 -  100 MHz
    R30B.PLL_N         =41;  // 63 - 100 MHz
...
*/
//-----------------------------------------------------------
// это тест для проверки фильтра sclf-44 , делаем гетеродин 410 МГц из 10 Мгц ref
...
//-----------------------
  ...
  R8B.TYPE0 = 0;//выход 8,
  R8B.TYPE1 = 0;//выход 9,
  R8B.TYPE2 = 5;//выход 10,2 -lvpecl (700 мВ)  LVPECL (2000 mVpp)
  R8B.TYPE3 = 5;//выход 11,2 -lvpecl (700 мВ)  LVPECL (2000 mVpp)
  R8B.ADR   = 8;
...
//----------------------
  ...
  R11B.NO_SYNC9     = 1;
  R11B.NO_SYNC11    = 1;
  R11B.SYNC_POL_INV = 1;//SYNC is active high
  R11B.SYNC_TYPE    = 1;//SYNC IO pin type pullup
  R11B.EN_PLL_XTAL  = 0;
  R11B.ADR      = 11;
//----------------------
  R12B.SYNC_PLL_DLD = 0;
